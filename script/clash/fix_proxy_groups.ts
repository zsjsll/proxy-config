...
脚本地址 https://accel.bigpig.online/https://raw.githubusercontent.com/zsjsll/proxy-config/refs/heads/dist/clash/fix_proxy_groups.js#isHidden=true&aiExclude=HK|RU|JP|OTHER&num=1&aiFilerMode=exclude&showCount=true
...
import type { AreaList } from "../tools/i18n";
import { nameConvert } from "../tools/i18n";
...
for (const proxyGroup of content["proxy-groups"]) {
  ...
  if (proxyGroup.name.includes("AI节点")) {
    if (proxyGroup["exclude-filter"]) {
      delete proxyGroup["exclude-filter"];
    }
    if (showCount) {
      proxyGroup.name = `${proxyGroup.name}(${aiSum})`;
    }
    if (aiFilerMode === "include") {
  ...
  if (showCount && ["自动选择", "手动选择"].some((kw) => proxyGroup.name.includes(kw))) {
